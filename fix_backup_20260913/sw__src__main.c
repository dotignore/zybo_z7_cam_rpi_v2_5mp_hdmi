...
#define BUILD_STAMP 0x5C6A904BU
...
int main(void)
{
	...
	xil_printf("[MAIN] HDMI 1080p60 up (bars until camera frames)\r\n");

	if (Imx219_Init() != 0) {
		xil_printf("[MAIN] FAIL: camera I2C / XCLR — keep HDMI bars\r\n");
		goto hang;
	}

	if (Imx219_Config1080pRaw10() != 0) {
		xil_printf("[MAIN] FAIL: IMX219 mode — keep HDMI bars\r\n");
		goto hang;
	}

	xil_printf("[MAIN] Starting S2MM (ring5) then IMX219 stream...\r\n");
	if (Imx219_Stream(1) != 0) {
		xil_printf("[MAIN] FAIL: stream on — keep HDMI bars\r\n");
		goto hang;
	}
