...
if {[catch {
  ...
  # Pulse all PS-to-PL clock-domain resets after loading a new bitstream.
  # A repeated JTAG load can otherwise leave the AXI video path stalled.
  ...
  mwr 0x02E00000 0
  mwr 0x04400000 0
  set raw [file join $proj pic HDMI_test_1920x1080_gbrg_raw10_u16le.bin]
  if {![file exists $raw] || [file size $raw] != 4147200} {die "RAW10 test asset missing/wrong size: $raw"}
  puts "Loading RAW10 u16le -> 0x04000000 ..."
  dow -data $raw 0x04000000
  mwr 0x04400000 0x52415732
  set gbr [file join $proj pic HDMI_test_1920x1080_gbr888.bin]
  if {[file exists $gbr]} {
    puts "Loading HDMI_test GBR888 -> 0x03000000 ..."
    if {[catch {dow -data $gbr 0x03000000} e]} {
      puts "dow -data failed ($e); mwr fallback"
      puts "Loaded [load_bin_at $gbr 0x03000000] bytes GBR888"
    } else {
      puts "dow -data HDMI_test GBR888 OK"
    }
    mwr 0x02E00000 0x47425231
    puts "TEST magic [mrd 0x02E00000]"
    puts "TEST pix0  [mrd 0x03000000]"
  } else {
