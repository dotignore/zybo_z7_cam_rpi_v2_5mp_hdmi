...
int main(void)
{
	...
	Xil_ICacheDisable();
	Xil_DisableMMU();
	/* Start the monotonic clock once, before any pipeline timeout or AE. */
	XTime_SetTime(0);
	...
	xil_printf("[MAIN] HDMI 1080p30 RAW10 test (2.000 s Global Timer), then camera\r\n");
...

	{
		...
		/* Allow HDMI lock acquisition before the measured hold. */
		usleep(1000000);
		XTime_GetTime(&t0);
