...
def main() -> int:
    ...
    # Read the actual split configuration tables in driver write order.
    tables = re.findall(r'WriteTableNamed\("[^"\n]*",\s*(Imx219Cfg\w+)', c)
    mode = [entry for name in tables for entry in parse_table(h, name)]
    ...
    checks = [
        ...
        (regs.get(0x0176) == 0x00 and regs.get(0x0177) == 0x00, "binning cal = average"),
        ...
        ("Xil_DCacheDisable" in main_c, "D-cache off for VDMA"),
        (main_c.find("Pipeline_Start") < main_c.find("Imx219_Stream(1)"),
         "stream on after pipeline"),
        ("usleep(1000000)" in c, "XCLR wait 1 s >= t5 6 ms"),
        (len(unlock) == 6, "access-unlock table present"),
    ]
    for cond, label in checks:
        (ok if cond else bad)(label)

    # Guard the two different bandwidth constraints: camera active rows and HDMI.
    vt_rate = 2 * 24000000 * u16(regs[0x0306], regs[0x0307]) / (
        regs[0x0304] * regs[0x0303] * regs[0x0301])
    line_length = u16(regs[0x0162], regs[0x0163])
    frame_length = u16(regs[0x0160], regs[0x0161])
    fps = vt_rate / (line_length * frame_length)
    (ok if abs(fps - 30) < 0.1 else bad)(f"camera rate {fps:.3f} fps")
    row_rate = 1920 * vt_rate / line_length
    (ok if row_rate < 90000000 else bad)(f"capture row rate {row_rate/1e6:.3f} Mpixel/s < 90")
    (ok if frame_length >= 1080 + 4 else bad)("frame timing includes vertical blanking")
    exposure = u16(regs[0x015A], regs[0x015B])
    (ok if exposure <= frame_length - 4 else bad)("exposure fits the frame")
